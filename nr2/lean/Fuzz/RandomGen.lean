...
def randMatrix (state : PrngState) (n : Nat) (lo hi : Float) :
    QRDecomp.FloatMat.FMat × PrngState := Id.run do
  let mut s := state
...
def randDiagDominant (state : PrngState) (n : Nat) (lo hi : Float) :
  ...
  for i in [:n] do
    let mut rowSum := 0.0
    for k in [:n] do
      rowSum := rowSum + (QRDecomp.FloatMat.get A i k).abs
    A' := QRDecomp.FloatMat.set A' i i (rowSum + 1.0)
  return (A', state')

def hilbertMatrix (n : Nat) : QRDecomp.FloatMat.FMat :=
  QRDecomp.FloatMat.fromFn n n fun i j =>
