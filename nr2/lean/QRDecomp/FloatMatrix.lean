...
/-- Vector addition. -/
def vecAdd (a b : FVec) : FVec :=
  Array.ofFn (n := a.size) fun i => a[i]! + b[i]!

/-- Scalar-vector multiplication. -/
def vecScale (c : Float) (v : FVec) : FVec :=
  v.map (c * ·)

/-- Dot product of two float vectors. -/
def dotVec (a b : FVec) : Float := Id.run do
  let mut s := 0.0
  for i in [:a.size] do
    s := s + a[i]! * b[i]!
  return s
...
def vecNorm2 (v : FVec) : Float :=
  (dotVec v v).sqrt

/-- Extract column j from a matrix. -/
def getCol (A : FMat) (j : Nat) : FVec :=
  Array.ofFn (n := nrows A) fun i => get A i j

/-- Set column j of a matrix. -/
def setCol (A : FMat) (j : Nat) (v : FVec) : FMat := Id.run do
  let mut A' := A
  for i in [:v.size] do
    A' := set A' i j v[i]!
  return A'
...
/-- Matrix transpose. -/
def transpose (A : FMat) : FMat :=
  let n := nrows A
  let m := ncols A
  fromFn m n fun i j => get A j i

end QRDecomp.FloatMat
