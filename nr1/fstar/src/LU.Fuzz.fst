...
(** Generate a diagonally dominant n x n matrix (guaranteed non-singular).
    A[i][i] = sum of |A[i][j]| for j != i, plus 1.
    This ensures strict diagonal dominance. *)
val rand_diag_dominant : n:pos -> prng_state -> prng_state & matrix n
let rand_diag_dominant n s =
  let (s1, m) = rand_matrix n s (-10) 11 in
  (* Make diagonally dominant: set diagonal to row sum of absolute values + 1 *)
...
(** Fuzz test: solve random systems A*x = b and verify.
    Generates a diagonally dominant A and random b, solves, verifies.
    Returns (passes, total, final_prng_state). *)
...
(** Run all fuzz tests across matrix sizes.
    Returns total passes and total tests. *)
